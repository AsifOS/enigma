...
static gdt_entry_t g_gdt[] = {
    ...
    GDT_ENTRY(0, 0xFFFFF, GDT_ACCESS_CODE_SELECTOR | GDT_ACCESS_CODE_READABLE 
        | GDT_ACCESS_PRIVILEGE_RING0 | GDT_ACCESS_PRESENT, 
        GDT_FLAGS_CODE_PROT | GDT_FLAGS_GRANULARITY_4K),
    
    /* Kernel 32-bi data segment. */

    GDT_ENTRY(0, 0xFFFFF, GDT_ACCESS_DATA_SELECTOR | GDT_ACCESS_DATA_WRITABLE
        | GDT_ACCESS_PRIVILEGE_RING0 | GDT_ACCESS_PRESENT, 
        GDT_FLAGS_CODE_PROT | GDT_FLAGS_GRANULARITY_4K)
};
...
void gdt_init() {
    ...
    gdt_load(&g_desc, GDT_CODE_SELECTOR, GDT_DATA_SELECTOR);
}
